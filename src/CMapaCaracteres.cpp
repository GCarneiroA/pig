...
CMapaCaracteres::CMapaCaracteres(char *nomeFonte, SDL_Renderer* renderer, int tamanhoFonte, int estilo, PIG_Cor corFonte, int outline, PIG_Cor corOutline)
{
  ...
  //  TTF_SetFontStyle(font, estilo);
    ...
    COffscreenRenderer *off = new COffscreenRenderer(altRend, largRend);
...
CMapaCaracteres::CMapaCaracteres(char *nomeFonte, SDL_Renderer* renderer, int tamanhoFonte, int estilo, char *fundoFonte, int outline, PIG_Cor corOutline)
{
    strcpy(nome, nomeFonte);
    tamFonte = tamanhoFonte;
    render = renderer;
...
//    TTF_SetFontStyle(font, estilo);
  ...
  //      surfaceTemp[j - PRIMEIRO_CAR] = TTF_RenderGlyph_Solid(font, (Uint16)j, PRETO);
    ...
    COffscreenRenderer *off = new COffscreenRenderer(altOffScreen, largOffScreen);
    off->PintarFundo(BRANCO);
    for (Uint16 j = PRIMEIRO_CAR; j < ULTIMO_CAR; j++) {
        glyphsT[j - PRIMEIRO_CAR] = SDL_CreateTextureFromSurface(off->GetRenderer(), surfaceTemp[j - PRIMEIRO_CAR]);
        SDL_FreeSurface(surfaceTemp[j - PRIMEIRO_CAR]);
