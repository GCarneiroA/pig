...
CAnimacao::CAnimacao(int px, int py, SDL_Renderer* renderizador, char *nomeArq, int usaGerenciadorTimer, int retiraFundo, int opacidade)
    : CObjeto(px, py, renderizador, nomeArq, retiraFundo, opacidade)
{
    souCopia = 0;
    offx = offy = 0;
    modoAtual = 0;
...
CAnimacao::CAnimacao(int px, int py, CAnimacao* base, int usaGerenciadorTimer)
    : CObjeto(px, py, base->renderer, base->nomeImagem)
{
    souCopia = 1;
    frameDoModo = -1;
...
void CAnimacao::InsereFrame(int idModo, int idFrame, float delayFrame)
{
...
int CAnimacao::Desenha()
{
    ...
    if (tempoFrame) {
        ...
        tempoDecorrido = CGerenciadorTimers::GetTempoDecorrido(idTimer);
    }

    if (tempoDecorrido > tempo) {
        frameDoModo++;
        if (frameDoModo >= modo->qtdFrames) {
            //testa se chegou ao ultimo frame do modo atual
            if (modo->loop) {
                frameDoModo=0;
            } else {
