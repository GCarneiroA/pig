...
CObjeto::CObjeto(int posx,int posy, SDL_Renderer* renderizador,char* nomeArquivo,int retiraFundo,int opacidade)
{
    ...
    memset(valoresFloat, 0, sizeof(int) * MAX_ATRIBUTOS_OBJETO);
    flip = SDL_FLIP_NONE;
...
void CObjeto::GetXY(int &x, int &y)
{
    x = this->x;
    y = this->y;
}

void CObjeto::SetColoracao(PIG_Cor cor)
{
    SDL_SetTextureColorMod(text, cor.r, cor.g, cor.b);
}

void CObjeto::Desenha(COffscreenRenderer *offRender)
{
    if (offRender == NULL) {
...
float CObjeto::GetAngulo()
{
    return angulo;
}

void CObjeto::SetFlip(PIG_Flip valor)
{
    flip = valor;
}

PIG_Flip CObjeto::GetFlip()
{
    return flip;
}

void CObjeto::SetPivo(int px, int py)
{
    pivoRelativo.x = px;
    pivoRelativo.y = py;
    //AtualizaBB();
...
void CObjeto::GetPivo(int &px, int &py)
{
    px = pivoRelativo.x;
    py = pivoRelativo.y;
}
...
void CObjeto::SetDimensoes(int altura, int largura)
{
    dest.h = alt = altura;
    dest.w = larg = largura;
    dest.x = x;
...
void CObjeto::GetDimensoes(int &altura, int &largura)
{
    altura = alt;
    largura = larg;
}

void CObjeto::GetDimensoesOriginais(int &altura, int &largura)
{
    altura = altOriginal;
    largura = largOriginal;
}

void CObjeto::SetOpacidade(int valor)
{
