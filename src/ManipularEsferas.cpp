...
Charger* InserirParticula(Charger* Lista, int TipoCarga, float Carga, float Px, float Py, float Vx, float Vy)
{
    ...
    if (Lista == NULL || Carga < Lista->Carga) {
        ...
        if (TipoCarga == 1) {
            Temp->Obj = CriarObjeto((char*)"imagens\\Positiva.bmp", Px, Py, TAMANHO_PARTICULA, TAMANHO_PARTICULA);
            Temp->TrajetoriaCor = ((PIG_Cor){0, 0, 255, 255});
        }
        if (TipoCarga == -1) {
            Temp->Obj = CriarObjeto((char*)"imagens\\Negativa.bmp", Px, Py, TAMANHO_PARTICULA, TAMANHO_PARTICULA);
