...
CJogo::CJogo(char *nome, int cursor)
{
    rodando = true;
...
inline PIG_Teclado CJogo::PegaTeclado()
{
    return teclado;
}
...
void CJogo::DesenhaRetangulo(int x, int y, int altura, int largura, PIG_Cor cor)
{
    SDL_Rect rect;
    rect.x = x;
...
void CJogo::DesenhaRetanguloVazado(int x, int y, int altura, int largura, PIG_Cor cor)
{
    SDL_Rect rect;
    rect.x = x;
    rect.y = ALT_TELA - (y + altura);
...
void CJogo::DesenhaLinhaSimples(int x1,int y1,int x2,int y2,PIG_Cor cor)
{
    SDL_SetRenderDrawColor(renderer,cor.r, cor.g, cor.b, 255);
    SDL_RenderDrawLine(renderer, x1, ALT_TELA - y1, x2, ALT_TELA - y2);
}

void CJogo::DesenhaLinhas(SDL_Point* points, int n, PIG_Cor cor)
{
    SDL_SetRenderDrawColor(renderer, cor.r, cor.g, cor.b, 255);
    SDL_RenderDrawLines(renderer, points, n);
}

void CJogo::DesenharPonto(int x, int y, PIG_Cor cor)
{
    //int i, j;
...
void CJogo::DesenharPontos(const SDL_Point *points, int count, PIG_Cor cor)
{
    SDL_SetRenderDrawColor(renderer,cor.r, cor.g, cor.b, 255);
    SDL_RenderDrawPoints(renderer, points, count);
}

void CJogo::DesenhaLinhasDisjuntas(int *x,int *y,int qtd,PIG_Cor cor)
{
    SDL_SetRenderDrawColor(renderer, cor.r, cor.g, cor.b, 255);
...
void CJogo::DesenhaLinhasSequencia(int *x,int *y,int qtd,PIG_Cor cor)
{
    SDL_SetRenderDrawColor(renderer, cor.r, cor.g, cor.b, 255);
...
int CJogo::CriaFonteFundo(char *nome, int tamanho, int estilo, char *arquivoFundo, int contorno, PIG_Cor corContorno)
{
...
int CJogo::CriaFonteNormal(char *nome, int tamanho, int estilo, PIG_Cor corLetra, int contorno, PIG_Cor corContorno)
{
    int aux = fontesUsadas;
...
inline int CJogo::GetLarguraPixels(char *str, int numFonte)
{
    return fontes[numFonte]->GetLarguraPixelsString(str);
}

inline void CJogo::EscreverCentralizada(char *str, int x, int y, int numFonte)
{
    fontes[numFonte]->EscreveStringCentralizado(str, x, y);
}

inline void CJogo::EscreverDireita(char *str, int x, int y, int numFonte)
{
    fontes[numFonte]->EscreveStringDireita(str, x, y);
}

inline void CJogo::EscreverEsquerda(char *str, int x, int y, int numFonte)
{
...
inline void CJogo::SalvaOffScreenBMP(char *nomeArquivo)
{
    if (offRenderer) {
...
inline void CJogo::PintaAreaOffScreen(int px, int py, PIG_Cor cor)
{
    if (offRenderer) {
...
inline void CJogo::PintaFundoOffScreen(PIG_Cor cor)
{
    if (offRenderer) {
...
inline void CJogo::DesenhaRetanguloOffScreen(int x1, int y1, int altura, int largura, PIG_Cor cor)
{
    if (offRenderer) {
...
inline void CJogo::DesenhaRetanguloVazadoOffScreen(int x1, int y1, int altura, int largura, PIG_Cor cor)
{
    if (offRenderer) {
...
inline void CJogo::DesenhaLinhaOffScreen(int x1, int y1, int x2, int y2, PIG_Cor cor)
{
    if (offRenderer) {
